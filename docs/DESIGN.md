...
**Bug found and fixed while building M5:** resuming a paused run (`graph.invoke(None, config=...)` after `update_state`) logged `Deserializing unregistered type amdra.schemas.* from checkpoint. This will be blocked in a future version` for every one of AMDRA's own Pydantic/Enum types — pre-existing since M1 (`cmd_run --review`) and M3b (`SqliteSaver`), just silently swallowed by pytest's default log capture (`--log-cli-level=WARNING` surfaces it; a bare script does too, which is how the M5 UI work found it). `graph/build.py` now has `memory_checkpointer()`/`sqlite_checkpointer()` — thin wrappers that configure `JsonPlusSerializer` with an explicit `allowed_msgpack_modules` list of AMDRA's own schema classes, rather than the blanket `allowed_msgpack_modules=True` bypass, consistent with the project's least-privilege posture elsewhere. `cli.py` and the M5 UI both use these now instead of constructing `MemorySaver()`/`SqliteSaver.from_conn_string()` directly.
...
- **M5 (presentation, done):** `src/amdra/ui/` (optional dependency, `pip install -e ".[ui]"`; `streamlit run src/amdra/ui/app.py`). **Investigate** page: pick a dispute, run it (offline by default, a real Claude call is an explicit one-case-at-a-time opt-in checkbox), inspect evidence grouped by kind (receipt PNGs rendered inline next to their OCR/vision text), the recommendation with each citation shown against the evidence block it quotes, verification results, injection flags (regex vs. M4c classifier distinguished), the audit trail, tool calls, and LLM usage/cost — with a working Approve/Reject flow for cases needing human review. **Eval Dashboard** page: browse a saved `evals/results/*.json` run — KPI tiles, accuracy by scenario, confusion matrix, authorization probe results, and a filterable case table. Both pages verified headlessly with `streamlit.testing.v1.AppTest` (not just read — the Approve/Reject/dispute-switch interactions were actually exercised, including the session-state caching that lets a paused human-review run survive Streamlit's rerun-the-whole-script-on-every-click model). Building this surfaced and fixed a real pre-existing bug — see §7.
