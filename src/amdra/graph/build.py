...
Two static wirings, picked at build time by `Settings.investigator` (never a runtime choice):
    ...
    "fixed" (default):
      intake -> gather_transactions -> gather_documents -> retrieve_policy -> decide -> verify
      gather_documents --(any receipt below ocr_confidence_threshold)--> vision_fallback -> retrieve_policy
                       \\--(otherwise)---------------------------------------------------> retrieve_policy

    "react" (M3c, opt-in ablation — see nodes.py investigate()):
      intake -> investigate -> retrieve_policy -> decide -> verify
...
from amdra.llm import Reasoner, VisionReasoner, make_reasoner
...
def build_graph(nodes: Nodes, checkpointer: Any = None, interrupt_for_review: bool = False):
    ...
    gather_names = ["investigate"] if react else ["gather_transactions", "gather_documents",
                                                   "vision_fallback"]
    for name in ["intake", *gather_names, "retrieve_policy", "decide", "verify", "review_gate",
    ...
    if react:
        ...
        g.add_edge("investigate", "retrieve_policy")
    else:
        ...
        g.add_edge("vision_fallback", "retrieve_policy")
...
class Agent:
    ...
    def create(cls, settings: Settings | None = None, reasoner: Reasoner | None = None,
               ...
               checkpointer: Any = None, interrupt_for_review: bool = False) -> Agent:
